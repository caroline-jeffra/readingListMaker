...
describe 'BookRepository', :book do
  ...
  describe '#create' do
    it 'Should create a book to the in-memory list' do
      repo = BookRepository.new(csv_path)
      new_book = Book.new(title: 'Title 4', author: 'Author Name', genre: 'Fiction', description: "It's a very short book about nothing at all!", isbn: 21_345_678_910)
    ...
    it 'Should set the new book id' do
      repo = BookRepository.new(csv_path)
      silly_book = Book.new(title: 'Japes for the Genteel', author: 'Bertie Worcester', genre: 'Comedy', description: 'What a silly book this is', isbn: 21_345_678_910)
      repo.create(silly_book)
      expect(silly_book.id).to eq(4)
      serious_book = Book.new(title: 'Topics for Serious Thought', author: 'Leo Tolstoy', genre: 'Drama', description: 'What a serious book this is', isbn: 21_345_678_911)
    ...
    it 'Every newly created book should be saved in a row in the CSV' do
      ...
      expect(repo.all[0].isbn).to eq(21_345_678_910)
      ...
      expect(repo.all[1].isbn).to eq(21_345_678_911)
